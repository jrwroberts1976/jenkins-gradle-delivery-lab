...
| **Daemon** | A long-running background service. | `jenkins-docker` runs an isolated Docker daemon for Jenkins builds. |
| **Digest** | A cryptographic identifier for the exact contents of a container image. | Docker shows SHA-256 digests when base images are pulled. |
...
| **Jenkins** | Automation server used to run the delivery pipeline. | It tests, packages, containerises, scans and later publishes the application. |
...
| **Private registry** | A server used to store container images that is not public. | Approved Homelab Defender images are intended to be pushed to the LAN registry. |
| **Publish** | Make an approved image available in the registry for later deployment. | The Publish image stage only runs when `PUBLISH_CONTAINER` is selected and earlier stages succeed. |
...
| Name | Meaning |
|---|---|
| **TestServer** | Host running Jenkins and other homelab services. |
