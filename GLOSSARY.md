...
| **Annotation (Kubernetes)** | Extra metadata attached to a Kubernetes object. | The deployment helper writes `kubernetes.io/change-cause=Jenkins build <number>` so the live release can be traced back to Jenkins. |
...
| **Automatic rollback** | Restoring the previous known-good release when a new deployment cannot be verified. | The K3s deployment helper remembers the current image before updating and restores it if rollout or health verification fails. Build 13 proved this path. |
...
| **Bind mount** | A host or daemon path made available inside a container. | Mounts give temporary containers access to required files or the isolated builder socket. |
...
| **Build number** | Jenkins' sequential identifier for a job run. | It becomes the release image tag, for example `homelab-defender:15`. |
...
| **Change cause** | Human-readable release information recorded on a Kubernetes Deployment revision. | The deployment helper records the Jenkins build number as `kubernetes.io/change-cause`. |
| **CI/CD** | Continuous Integration / Continuous Delivery: automatically testing, packaging and delivering changes. | Jenkins performs the complete gated path through Kubernetes deployment and health verification. |
...
| **ClusterIP** | The default Kubernetes Service type, reachable only from inside the cluster. | Homelab Defender uses a ClusterIP so the application stays private during testing and verification. |
...
| **CRI** | Container Runtime Interface: the Kubernetes API used to talk to a container runtime. | K3s talks to containerd through CRI; `crictl` was used to inspect the build-15 digest and local image ID independently. |
...
| **crictl** | A command-line tool for inspecting and testing CRI-compatible container runtimes. | `sudo k3s crictl images --digests` was used to distinguish the build-15 registry digest from containerd's local image/config ID. |
...
| **Deployment health verification** | A post-rollout check that confirms the released application is reachable through its intended service path. | After Kubernetes reports rollout success, the deployment helper checks `/healthz` through the ClusterIP Service and retries up to 15 times. |
| **Desired-state reconciliation** | Updating the Git-owned declaration after a validated runtime change so Git again describes what is approved. | After build 15 succeeded, its tag and digest were reconciled into `kubernetes-homelab` through pull request `#11`. |
| **Digest** | A cryptographic identifier for exact container image content. | Builds 12–14 shared digest `sha256:325b28f…eeb4a`; build 15 was independently verified as `sha256:2154a188…ad940b`. The approved Kubernetes manifest records tag plus digest. |
...
| **Docker Registry HTTP API V2** | The standard HTTP API used by Docker clients to talk to a registry. | Requests to `/v2/` and repository tag endpoints were used to verify connectivity, authentication and published images. |
...
| **Exit code** | A number returned by a command to say whether it succeeded or failed. | Trivy returns non-zero when policy is breached; the deployment helper also returns non-zero when rollout or health verification fails. |
...
| **Git** | Version-control software that records changes to files. | Application source, pipeline logic, deployment helper, Kubernetes desired state and operational documentation are versioned in Git. |
| **GitHub** | The hosted repository service containing the project source and history. | Jenkins fetches the public application repository from GitHub and release reconciliation is reviewed through pull requests. |
| **Grafana** | Dashboard and alerting software. | Live Grafana on `ids-01` hosts the `Homelab Defender Kubernetes Operations` dashboard and two Defender-specific alert rules. |
...
| **hosts.toml** | containerd registry-host configuration describing endpoints and capabilities. | K3s generated an entry containing the internal HTTP registry host after loading `registries.yaml`. |
| **HTTP 401 Unauthorized** | An HTTP response meaning a service was reached but authentication was missing or rejected. | A registry `401` proved network reachability before valid credentials returned HTTP 200. |
| **htpasswd** | A file format and command used to store HTTP Basic-authentication usernames and password hashes. | TestServer's registry reads `/etc/docker/registry/htpasswd`. |
| **Image ID** | A runtime-local identifier for an image/config object. | containerd reported a build-15 local image ID beginning `4248f415...`; it is not the same thing as the registry digest. |
| **Image pull** | Downloading a container image from a registry into a container runtime. | K3s/containerd first proved a manual pull with build 12 and later pulled build 15 through the automated release path. |
...
| **Image tag** | A human-readable label attached to a container image. | Jenkins uses the build number, for example `homelab-defender:15`, instead of `latest`. |
| **Immutable release** | A release identified by content rather than only by a changeable tag. | The approved build-15 desired state records `:15@sha256:2154a188…ad940b`. |
| **Jammy** | Ubuntu 22.04 LTS, whose codename is Jammy Jellyfish. | The runtime base is `eclipse-temurin:21-jre-jammy`. |
| **Java DB (Trivy)** | Trivy's vulnerability database used for Java-related analysis. | Build 15 refreshed this database; the persistent cache metadata showed a three-day next-update window. |
...
| **JRE** | Java Runtime Environment: components needed to run Java software. | The final application image uses a smaller Java 21 runtime image. |
| **Jenkins** | Automation server used to run the delivery pipeline. | It tests, packages, containerises, scans, publishes and deploys approved application releases. |
...
| **kube-state-metrics** | A service that turns Kubernetes object state into Prometheus metrics. | The existing instance at `192.168.2.211:8080` provides Defender deployment, pod, readiness, restart and image identity series. |
| **Kubernetes** | A platform for running and managing containerised applications. | It currently runs the validated build-15 workload in the isolated `homelab-defender-test` namespace. |
...
| **Liveness probe** | A Kubernetes health check used to decide whether a running container should be restarted. | The Homelab Defender Deployment checks `/healthz` on port 8080 after startup. |
| **Mirror (container registry)** | A registry endpoint a runtime can use for requests to a named registry. | K3s `registries.yaml` defines the internal HTTP endpoint as the mirror for `192.168.2.220:5000`. |
...
| **Pod** | Kubernetes' smallest deployable unit, usually containing one application container. | Build 14 was the first fully automated healthy release; the current build-15 pod was validated Running with zero restarts. |
...
| **Prometheus** | A time-series monitoring system. | Prometheus on `ids-01` stores Defender metrics from kube-state-metrics and feeds the live Grafana dashboard and alert rules. |
| **Publish** | Make an approved image available in the registry for deployment. | The Publish image stage runs only when `PUBLISH_CONTAINER` is selected and all earlier gates succeed. |
...
| **Registry credential** | Username/password used to authenticate to the private registry. | Jenkins stores publishing credentials under `homelab-registry`; K3s stores a separate pull credential in root-owned configuration. |
| **registries.yaml** | K3s configuration file for private registry endpoints and authentication. | `/etc/rancher/k3s/registries.yaml` defines the internal HTTP endpoint and K3s pull credentials. |
| **Registry fallback** | Trying another registry endpoint when the first endpoint does not complete a request. | K3s initially fell back to HTTPS before its internal registry configuration was loaded; during build 15 Trivy also used its own fallback DB repository after the primary mirror returned `BLOB_UNKNOWN`. |
| **Release** | A build that has passed its required controls and is deliberately put into service. | Build 15 is the current approved release. |
| **Rollback** | Returning an application to a previous known-good version. | The automated helper restores the previously running image after a failed rollout or persistent health failure; Kubernetes `rollout undo` is also documented for manual recovery. |
...
| **Service account (general)** | A non-human account intended for application or automation use. | `jenkins-ci` publishes images, K3s has a separate registry pull identity, and `jenkins-deploy` is used only for deployment requests. |
...
| **SSH key credential** | A private/public key pair used for non-password SSH authentication. | Jenkins stores the deployment private key under credential ID `k3s-deploy-ssh`; the matching public key is constrained by a forced command. |
...
| **sudo rule** | A policy controlling which commands one local account may run as another user. | `jenkins-deploy` has a narrow NOPASSWD rule allowing only `/usr/local/sbin/deploy-homelab-defender`. |
| **Temurin** | Eclipse Adoptium's OpenJDK distribution. | The final runtime is based on `eclipse-temurin:21-jre-jammy`. |
| **TLS** | Transport Layer Security: encryption plus identity verification for network connections. | Jenkins connects securely to the isolated Docker daemon on port 2376 using client certificates. The private registry uses LAN HTTP with authentication and firewall restrictions. |
...
| **Trivy cache** | Persistent storage used to reuse downloaded Trivy vulnerability databases. | The DinD volume `trivy-cache` contained about `2.6G` after build 15, confirming the cache is persistent and healthy. |
| **Vulnerability database** | Structured data mapping software versions to known security problems. | Trivy downloads and caches its databases and compares image contents against them. |
| **Workspace** | The directory Jenkins uses for a job's checked-out files and build output. | The `main` job runs inside its Jenkins workspace before artifacts and images are produced. |
...
| **ids-01** | Host running the live Grafana and Prometheus path used for Defender operational monitoring. |
...
| **`BUILD_CONTAINER`** | Jenkins parameter that asks the pipeline to build and scan an image without publishing or deploying it. |
...
| **`ops/deploy-homelab-defender`** | Version-controlled source of the root-owned K3s deployment and automatic rollback helper. |
| **`kubernetes-homelab`** | Repository that owns the approved Homelab Defender Kubernetes desired state, including immutable release identity. |
